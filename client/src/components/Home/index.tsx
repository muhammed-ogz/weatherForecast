...
const Home = () => {
  ...
  return (
    <div className="items-center min-h-screen bg-gray-900 text-white">
      <div className="absolute flex justify-between left-40 m-4 shadow-lg shadow-black w-200 h-64 bg-gray-700 rounded-4xl p-8">
        <div className="text-4xl ml-1 leading-normal inline-block">
          İstanbul
        </div>
        ...
        />
        <div className="absolute bottom-0 left-0 m-9  w-145 h-24">
          <ul>
            {Array.from({ length: 6 }).map((_, index) => (
              <li
                key={index}
                className="inline-block w-20 h-20 m-2 bg-gray-600 rounded-lg text-center leading-20"
              >
                Box {index + 1}
              </li>
            ))}
      ...
      <div className="absolute flex justify-between right-10 m-4 shadow-lg shadow-black w-110 h-170 bg-gray-700 rounded-4xl p-8">
